...
def pitch_srh(wave, fs, f0min, f0max, hopsize=10):
    ...
    specMat = np.abs(np.fft.fft(frameMatWinMean, fs, axis=0)[:fs // 2])
...
def lpcresidual(x, L, shift, order):
    ...
    while stop <= len(x):
        ...
        A = np.polyfit(np.arange(len(segment)), segment, order)
        LPCcoeff[:, n] = A

        inv = lfilter([1], A, segment)
...
def SRH(specMat, nHarmonics, f0min, f0max):
    ...
    plusIdx = (plusIdx - 1) % specMat.shape[0]
    subtrIdx = (subtrIdx - 1) % specMat.shape[0]
